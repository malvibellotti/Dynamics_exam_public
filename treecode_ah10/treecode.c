...
/*
 * HERNQUIST_ACCELERATION: compute acceleration due to Hernquist potential with enclosed mass.
 */
local void hernquist_acceleration(bodyptr p, real M, real a)
{
    real r_mag, r_dot, M_H_enclosed, prefactor;
    vector acc, pos_unit;

    // Compute the magnitude of the 3D position vector r
    DOTVP(r_dot, Pos(p), Pos(p));  
    r_mag = sqrt(r_dot);  // r = sqrt(x^2 + y^2 + z^2)
    
    // Compute the enclosed mass for the Hernquist halo
    M_H_enclosed = M * (r_mag * r_mag) / ((r_mag + a) * (r_mag + a));

    real G_const =  1;  

    // Compute the prefactor for acceleration
    prefactor = -G_const * M_H_enclosed / (r_mag * r_mag);  // a = -GM(r)/r^2

    // Calculate the unit vector in the direction of r
    DIVVS(pos_unit, Pos(p), r_mag);  // pos_unit = Pos(p) / r_mag

    // Compute the acceleration vector
    MULVS(acc, pos_unit, prefactor);  // acc = prefactor * pos_unit

    // Add the Hernquist acceleration to the particle's existing acceleration
...
local void treeforce(void)
{
    ...
    real a_scale = 10;  // Scale radius in parsecs
