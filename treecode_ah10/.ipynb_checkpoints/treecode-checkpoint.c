...
/*let's add an hernquist acceleration*/
/*
 * HERNQUIST_ACCELERATION: compute acceleration due to Hernquist potential.
 */

// Modify the Hernquist acceleration function to use the correct DOTVP macro
local void hernquist_acceleration(bodyptr p, real M, real a)
{
    real r_mag, r_dot;
    vector acc, pos_unit;

    // DOTVP requires 3 arguments: destination, first vector, second vector
    DOTVP(r_dot, Pos(p), Pos(p));  
    r_mag = sqrt(r_dot);  // magnitude of position vector
    
    // You'll need to define G somewhere, typically in a constants header
    // For astrophysical simulations, G is often defined as 4.302e-3 in (kpc/M_sun)(km/s)^2
    real G_const =  1;  
    
    // Hernquist acceleration formula
    real prefactor = -G_const * M / pow(r_mag + a, 2);
    //real prefactor = -1;

     // Calculate unit position versor
    DIVVS(pos_unit, Pos(p), r_mag);

    MULVS(acc, pos_unit, prefactor);  // scale unit position vector by prefactor
    
    // Add Hernquist acceleration to existing acceleration
    ADDV(Acc(p), Acc(p), acc);
}

// Modify the treeforce() function to include Hernquist potential
local void treeforce(void)
{
    bodyptr p;
    real M_halo = 1000;  // Total halo mass in solar masses
    real a_scale = 6.0;  // Scale radius in parsecs
    ...
    // Add Hernquist halo acceleration to each body
    for (p = bodytab; p < bodytab+nbody; p++)
        hernquist_acceleration(p, M_halo, a_scale);
